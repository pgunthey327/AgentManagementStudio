...
// ─────────────────────────────────────────────
function populateAgentGroups(extensionPath) {
...
async function copySelectedAgents(selectedAgents, workspaceRoot, extensionPath) {
...
// ─────────────────────────────────────────────
function mergeSpecs(defaultSpecs, userSpecs) {
...
function loadCustomAgentData(extensionPath, workspaceRoot) {
    const resourcesDir = path.join(extensionPath, 'resources');
    const userDir = path.join(workspaceRoot, '.github', 'agent-specs');
    const defaultRoles = loadJson(path.join(resourcesDir, 'roles.json'));
    const defaultResponsibilities = loadJson(path.join(resourcesDir, 'responsibilities.json'));
    const defaultSkills = loadJson(path.join(resourcesDir, 'skills.json'));
    const defaultTools = loadJson(path.join(resourcesDir, 'tools.json'));
    const defaultInstructions = loadJson(path.join(resourcesDir, 'instructions.json'));
    const userRoles = loadJson(path.join(userDir, 'roles.json'));
    const userResponsibilities = loadJson(path.join(userDir, 'responsibilities.json'));
    const userSkills = loadJson(path.join(userDir, 'skills.json'));
    const userTools = loadJson(path.join(userDir, 'tools.json'));
    const userInstructions = loadJson(path.join(userDir, 'instructions.json'));
    return {
...
function saveNewSpec(specType, newSpec, workspaceRoot, extensionPath) {
    const userDir = path.join(workspaceRoot, '.github', 'agent-specs');
    ...
    const resourceData = loadJson(resourcePath);
...
// Markdown generation
// ─────────────────────────────────────────────
function generateCustomAgentFile(data, workspaceRoot, extensionPath) {
...
// Webview
// ─────────────────────────────────────────────
function getWebviewContent(webview, extensionUri, allSpecs) {
...
async function showCustomAgentWebview(context, workspaceRoot) {
    ...
    panel.webview.onDidReceiveMessage(async (message) => {
        switch (message.command) {
            ...
            case 'createAgent': {
                try {
                    const filePath = generateCustomAgentFile(message.data, workspaceRoot, context.extensionPath);
                    vscode.window.showInformationMessage(`Agent created: ${path.basename(filePath)}`, 'Open File').then((choice) => {
                        if (choice === 'Open File') {
            ...
            case 'addNewSpec': {
                try {
                    ...
                    vscode.window.showErrorMessage(`Failed to save spec: ${err.message}`);
                }
                break;
            }
            case 'cancel':
...
// Main orchestrator
// ─────────────────────────────────────────────
async function populateGitHubFolder(workspaceRoot, extensionPath, context) {
    ...
    vscode.window.showInformationMessage(`AI Agent Suite: ${parts.join(', ')} → .github/agents/`);
}
// ─────────────────────────────────────────────
